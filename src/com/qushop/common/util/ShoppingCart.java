...
public class ShoppingCart {
	...
	public void addCart(Product product,Integer addCount,Integer cartType,Product_ext_shop product_ext_shop,String leaseCycle){
...
//			shopMap.put("shopproduct", product_ext_shop);
			...
			//type位0代表商城商品
			if(cartType.intValue()==0){
				Date nowdate = new Date();
				if(product_ext_shop.getPromoteflag() == 1 && nowdate.after(product_ext_shop.getPromoteStartTime())  && nowdate.before(product_ext_shop.getPromoteEndTime())){
			...
			//type位1代表合作社商品
			else if(cartType.intValue()==1)
			{
				shopMap.put("shopPrice", product.getShopPrice());
			}//type 2 代表 租赁
			else if(cartType.intValue()==2){
