...
import com.qushop.alipay.util.AlipayCore;
...
public class AlipayController {
	...
	public Object pay(HttpServletRequest request, HttpServletResponse response, String orderId, Integer orderType)
		...
		// 必填，不能修改
		// 服务器异步通知页面路径
		String notify_url = "http://localhost:8080/musicians/alipay/notifyUrl.action";
		// 需http://格式的完整路径，不能加?id=123这类自定义参数

		// 页面跳转同步通知页面路径
		String return_url = "http://localhost:8080/musicians/alipay/returnUrl.action";
		...
		// 付款金额
		  BigDecimal   bd   =   new   BigDecimal(order_lists.get(0).getTotalamt());   
	...
	//	String body = new String(request.getParameter("WIDbody").getBytes("ISO-8859-1"),"UTF-8");
		...
		String receive_address = order_lists.get(0).getUserAddress().getStateId()
				+ order_lists.get(0).getUserAddress().getCityId() + order_lists.get(0).getUserAddress().getStreet();
	...
	//	sParaTemp.put("body", body);
		...
		String buildRequestMysign =  AlipayCore.createLinkString(AlipaySubmit.buildRequestPara(sParaTemp));

		//return sHtmlText;
		return "redirect:https://mapi.alipay.com/gateway.do?"+buildRequestMysign;
	}
	/**
	 * 
	 * @param request
	 ...
	 *            支付宝交易号
	 * @param trade_status
	 *            交易状态 获取支付宝的通知返回参数，可参考技术文档中页面跳转同步通知参数列表(以上仅供参考)
	 * @return
	...
	public Object returnUrl(HttpServletRequest request) throws UnsupportedEncodingException {
		...
		if (verify_result) {// 验证成功
			...
			order_listService.orderPayment(out_trade_no, 1);

			// 该页面可做页面美工编辑
			return "redirect:/order/orderList/orderList.do";
	...
	public String notifyUrl(HttpServletRequest request, String out_trade_no, String trade_no, String trade_status)
		...
		List<Order_list> ordersList = order_listService.getOrder_listDetail(out_trade_no,
				orderType == 3 ? true : false);

		if (AlipayNotify.verify(params)) {// 验证成功
			//////////////////////////////////////////////////////////////////////////////////////////
			// 请在这里加上商户的业务逻辑程序代码
			// List<Order_list> ordersList =
			////////////////////////////////////////////////////////////////////////////////////////// order_listService.getOrder_listDetail(out_trade_no,
			////////////////////////////////////////////////////////////////////////////////////////// orderType==3?true:false);
			...
			if (trade_status.equals("WAIT_BUYER_PAY")) {
				...
				if (orderList.getOrderType() == 2) {
					...
					if (details != null && details.size() > 0) {
						...
						List<Product_ext_groupBuy> groupBuys = groupBuyService.getgroupBuyProductByMethod(5,
								groupbuyId);
	...
	@ResponseBody
	public Object sendGoods(String orderId, String waybill, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		// 支付宝交易号
		String trade_no = order_listService.getOrder_listDetail(orderId, false).get(0).getThirdpartorderno();// new
