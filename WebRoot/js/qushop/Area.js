...
function changeComplexProvinceForEdit(f, k, e, d, j) {
	var c = changeComplexProvinceForEdit.arguments;
	var h = document.getElementById(e);
	var g = document.getElementById(d);
	var b = 0;
	var a = 0;
...
function changeCity(c, a, t) {
	...
	if (_d) {
		...
		for (var i = c * 100; i < _d.length; i++) {
			if (_d[i] == undefined)
				continue;
			var selected = i == t ? " selected='selected'" : "";
			str += "<option value='" + i + "'" + selected + " >" + _d[i]
					+ "</option>";
		}
		$("#" + a).html(str);
	}
