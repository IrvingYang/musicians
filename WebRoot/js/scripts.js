	...
	(function(){
		...
		if(ncc.length){
			var nc = ncc.owlCarousel({
		 		itemsCustom : [[1199,4],[992,4],[768,3],[575,2],[300,1]],
		 		autoPlay : false,
		 		slideSpeed : 1000,
		 		autoHeight : true
		 	});
	...
	(function(){
		...
		$('.custom_select').each(function(){
			...
			if($(this).find('[data-filter]').length){
				...
				for(var i = 0,len = select.children('option').length;i < len;i++){
					list.append('<li class="tr_delay_hover" key='+select.children('option').eq(i).attr("key")+' value='+select.children('option').eq(i).val()+'>'+select.children('option').eq(i).text()+'</li>')
				}
			}
			select.hide();
...
//						alert($(e.target).hasClass('popup_wrap'));
...
jQuery(function(){
    ...
    var point = new BMap.Point(102.2726, 27.9006);  // 需要标注的位置的经纬度  、、西昌市石塔街19号//102.1114660877,27.8571076151
    map.centerAndZoom(point, 17);  // 中心位置和缩放倍数
